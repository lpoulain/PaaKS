...
func (p *parser) doParse() (Query, error) {
	for {
		...
		case stepUpdateValue:
			...
			if ln == 0 {
				return p.query, fmt.Errorf("at UPDATE: expected quoted value: " + quotedValue)
			}
		...
		case stepWhereValue:
			...
			if isIdentifier(identifier) {
				...
				quotedValue, ln := p.peekQuotedStringWithLength()
				if ln == 0 {
					return p.query, fmt.Errorf("at WHERE: expected quoted value: " + quotedValue)
				}
...
func (c Condition) getSql() string {
	...
	if c.Operand1IsField {
		...
		sql += fmt.Sprintf("'%s'", c.Operand1)
	}

	sql += fmt.Sprintf(" %s ", OperatorString[c.Operator])

	if c.Operand2IsField {
		sql += c.Operand2
	} else {
		sql += fmt.Sprintf("'%s'", c.Operand2)
	}
...
func (q Query) getSql() string {
	switch q.Type {
	case Select:
		sql := fmt.Sprintf("SELECT %s FROM %s", strings.Join(q.Fields, ", "), q.TableName)
		if len(q.Conditions) == 0 {
			return sql
		}

		sql += " WHERE " + strings.Join(Map[Condition, string](q.Conditions, func(c Condition) string { return c.getSql() }), " AND ")
